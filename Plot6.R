...
nei.vehicles <- nei.vehicles %>% 
  filter(fips == "24510"| fips == "06037"); # Filter for Baltimore City or Los Angeles County
...
nei.vehicles <- merge(x = nei.vehicles, 
                      y = scc.vehicles[, c("SCC", "SCC.Level.Two")], 
...
ggplot(nei.vehicles, aes(x = factor(year), y = Annual.Total, fill = SCC.Level.Two)) +
  ...
  facet_grid(fips ~ SCC.Level.Two,scales = "free") + 
  xlab("Year") +
  ylab(expression("Total Tons of PM2.5 Emissions")) + 
  ggtitle(expression(atop("Total Tons of PM2.5 Emissions from Motor Vehicle Sources", 
