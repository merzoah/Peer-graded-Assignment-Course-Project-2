...
nei.coal$type <- factor(nei.coal$type, 
                        levels = c("TOTAL", "ON-ROAD", "NON-ROAD", "POINT", "NONPOINT")); # Re-order factor levels to they plot in the order we wish
...
ggplot(nei.coal, aes(x = factor(year), y = Annual.Total, fill = type)) +
  ...
  ggtitle(expression(atop("Total Tons of PM2.5 Emissions in the United States", 
